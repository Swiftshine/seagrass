...
pub fn check_data_types() -> Result<()> {
    ...
    assert!(matches!(first.borrow().copy_value(), RuntimeValue::U32(1)));
    assert!(matches!(second.borrow().copy_value(), RuntimeValue::S32(2)));
    assert!(matches!(third.borrow().copy_value(), RuntimeValue::S32(3)));
    Ok(())
}
