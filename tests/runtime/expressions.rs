...
pub fn addition() -> Result<()> {
    ...
    assert!(matches!(*result_plus, RuntimeValue::S32(3)));
    assert!(matches!(*result_minus, RuntimeValue::S32(1)));
    Ok(())
}
...
pub fn multiplication() -> Result<()> {
    ...
    assert!(matches!(*result_mul, RuntimeValue::S32(6)));
    assert!(matches!(*result_div, RuntimeValue::S32(2)));
    Ok(())
}
